...
-- We need to alias the return table columns to avoid conflict with table columns in queries

-- 1. Drop the function first to ensure clean state
DROP FUNCTION IF EXISTS get_aggregated_usage_logs(text, text, integer, integer, text, text);

-- 2. Create the function with unambiguous return column names (prefixed with 'ret_')
-- OR better yet, alias the tables inside the query to resolve ambiguity
...
BEGIN
  ...
  user_with_details AS (
    SELECT 
      ua.user_id,  -- Explicitly select from alias
      ua.total_prompt_tokens,
      ua.total_completion_tokens,
      ua.total_tokens,
      ua.total_estimated_cost,
      ua.usage_count,
      ua.earliest_activity,
      ua.latest_activity,
      ...
      COALESCE(
        (SELECT full_name FROM user_profiles WHERE user_profiles.user_id = ua.user_id LIMIT 1),
        (SELECT raw_user_meta_data->>'full_name' FROM auth.users WHERE auth.users.id = ua.user_id),
        (SELECT raw_user_meta_data->>'name' FROM auth.users WHERE auth.users.id = ua.user_id),
        'User ' || SUBSTRING(ua.user_id::text, 1, 8)
      )::text AS user_name,
      COALESCE(
        (SELECT email::text FROM auth.users WHERE auth.users.id = ua.user_id),
  ...
  user_activity AS (
    SELECT 
      ...
      uwd.user_name,
      uwd.user_email,
      uwd.total_prompt_tokens,
      uwd.total_completion_tokens,
      uwd.total_tokens,
      uwd.total_estimated_cost,
      uwd.usage_count,
      uwd.earliest_activity,
      uwd.latest_activity,
      uwd.has_completed_payment,
      uwd.user_type,
...
COMMENT ON FUNCTION get_aggregated_usage_logs IS 'Aggregates usage logs with sorting support - fixed ambiguous column references';
