-- Add time_range parameter to get_aggregated_usage_logs
-- Drops the previous 6-argument version and creates a 7-argument version

-- Drop the 6-argument version
DROP FUNCTION IF EXISTS get_aggregated_usage_logs(text, text, integer, integer, text, text);

-- Create the updated function with 7 arguments
CREATE OR REPLACE FUNCTION get_aggregated_usage_logs(
  ...
  sort_by text DEFAULT 'latest_activity',
  time_range text DEFAULT 'all'  -- 'all', 'weekly', 'monthly'
)
RETURNS TABLE (
  user_id uuid,
  user_name text,
...
BEGIN
  ...
  WITH filtered_logs AS (
    ...
    WHERE 
      CASE
        WHEN time_range = 'weekly' THEN created_at >= (now_timestamp - interval '7 days')
        WHEN time_range = 'monthly' THEN created_at >= (now_timestamp - interval '30 days')
        ELSE true
      END
  ),
  user_aggregates AS (
    SELECT 
      fl.user_id,
      SUM(fl.total_prompt_tokens) AS total_prompt_tokens,
      SUM(fl.total_completion_tokens) AS total_completion_tokens,
      SUM(fl.total_tokens) AS total_tokens,
      SUM(fl.estimated_cost) AS total_estimated_cost,
      COUNT(*) AS usage_count,
      MIN(fl.created_at) AS earliest_activity,
      MAX(fl.created_at) AS latest_activity
...
-- Grant permissions
GRANT EXECUTE ON FUNCTION get_aggregated_usage_logs(text, text, integer, integer, text, text, text) TO authenticated;
GRANT EXECUTE ON FUNCTION get_aggregated_usage_logs(text, text, integer, integer, text, text, text) TO anon;
GRANT EXECUTE ON FUNCTION get_aggregated_usage_logs(text, text, integer, integer, text, text, text) TO service_role;

COMMENT ON FUNCTION get_aggregated_usage_logs IS 'Aggregates usage logs with sorting and time range support';
