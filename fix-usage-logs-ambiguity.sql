-- Fix for "function name is not unique" error
-- We explicitly drop each possible signature of the function to remove ambiguity

-- 1. Drop the original 4-argument version
DROP FUNCTION IF EXISTS get_aggregated_usage_logs(text, text, integer, integer);

-- 2. Drop the 5-argument version (with user_type_filter)
DROP FUNCTION IF EXISTS get_aggregated_usage_logs(text, text, integer, integer, text);

-- 3. Drop the 6-argument version (with sort_by) if it exists partially
DROP FUNCTION IF EXISTS get_aggregated_usage_logs(text, text, integer, integer, text, text);

-- Now create the fully updated function with 6 arguments
