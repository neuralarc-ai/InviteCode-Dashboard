-- Update function to support internal/external user filtering
-- Internal users: @he2.ai and @neuralarc.ai
...
CREATE OR REPLACE FUNCTION get_aggregated_usage_logs(
  ...
  user_type_filter text DEFAULT 'external'  -- New parameter: 'internal', 'external', or 'all'
)
RETURNS TABLE (
  user_id uuid,
  user_name text,
  ...
  user_type text  -- New field: 'internal' or 'external'
)
LANGUAGE plpgsql
SECURITY DEFINER
...
-- Grant permissions
GRANT EXECUTE ON FUNCTION get_aggregated_usage_logs(text, text, integer, integer, text) TO authenticated;
GRANT EXECUTE ON FUNCTION get_aggregated_usage_logs(text, text, integer, integer, text) TO anon;
GRANT EXECUTE ON FUNCTION get_aggregated_usage_logs(text, text, integer, integer, text) TO service_role;

COMMENT ON FUNCTION get_aggregated_usage_logs IS 'Aggregates usage logs with support for internal/external user filtering - @he2.ai and @neuralarc.ai are internal users';
