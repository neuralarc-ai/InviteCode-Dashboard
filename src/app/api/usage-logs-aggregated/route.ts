...
async function fetchAndAggregateManually(
  ...
  if (!usageLogs)
    return { data: [], totalCount: 0, grandTotalTokens: 0, grandTotalCost: 0 };
  ...
  let aggregatedUsers = Array.from(userStats.values()).map((user) => {
    const daysSinceLastActivity = Math.floor(
      (now.getTime() - user.latest_activity.getTime()) / (1000 * 60 * 60 * 24),
    );
    ...
    const activity_score = Math.floor(
      recency_score * 0.5 + frequency_score * 0.3 + volume_score * 0.2,
    );
  ...
  aggregatedUsers = aggregatedUsers.filter((user) => {
    // Activity Filter
    if (activityFilter !== "all" && user.activity_level !== activityFilter)
      return false;
  ...
  const grandTotalTokens = aggregatedUsers.reduce(
    (sum, u) => sum + u.total_tokens,
    0,
  );
  const grandTotalCost = aggregatedUsers.reduce(
    (sum, u) => sum + u.total_estimated_cost,
    0,
  );
...
export async function POST(request: Request) {
  try {
    ...
    const { data: aggregatedData, error: rpcError } = await supabaseAdmin.rpc(
...
function createResponse(data: any[] | null, page: number, limit: number) {
  ...
  const grandTotalTokens =
    data && data.length > 0 ? data[0].grand_total_tokens : 0;
