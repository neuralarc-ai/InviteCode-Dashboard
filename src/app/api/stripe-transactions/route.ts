  ...
  try {
    ...
    const transformedCharges: StripeCharge[] = charges.data.map((charge) => {
      ...
      return {
        ...
        paymentMethodDetails: Object.keys(paymentMethodDetails).length > 0 ? paymentMethodDetails : undefined,
    ...
    return NextResponse.json({
      ...
      // For pagination
      firstChargeId: transformedCharges.length > 0 ? transformedCharges[0].id : null,
      lastChargeId: transformedCharges.length > 0 ? transformedCharges[transformedCharges.length - 1].id : null,
    });
  } catch (error) {
