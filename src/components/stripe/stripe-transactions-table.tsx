...
import type { SortField, SortDirection } from "@/hooks/use-stripe-transactions";
...
export function StripeTransactionsTable() {
  ...
  const formatStripeAmount = (amount: number, currency: string = "usd"): string => {
    ...
    <div className="space-y-5 sm:space-y-4">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div className="space-y-1">
          ...
          <p className="text-sm text-muted-foreground">
            Payment charges from Stripe {environment === "test" ? "(Test Mode)" : "(Production)"}
          </p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
          <div className="flex items-center gap-2">
            <span className={`text-sm font-medium ${environment === "test" ? "text-foreground" : "text-muted-foreground"}`}>
              Test
            </span>
            <Switch
            ...
            />
            <span className={`text-sm font-medium ${environment === "production" ? "text-foreground" : "text-muted-foreground"}`}>
              Production
            </span>
          </div>
      ...
      {error && (
        <div className="rounded-lg border border-red-500/50 bg-red-500/10 p-4 flex items-center gap-3">
          ...
          <div>
            <p className="font-medium text-red-700 dark:text-red-400">Error loading transactions</p>
            ...
            >
              ...
              <div className="flex flex-wrap sm:flex-nowrap items-center justify-center md:justify-evenly gap-3 sm:gap-4">
                ...
                <div className="font-bold text-xl whitespace-nowrap">
                  {formatStripeAmount(charge.amount, charge.currency)}
                  {charge.amountRefunded > 0 && (
                    <span className="text-sm text-muted-foreground ml-2">
                      (Refunded: {formatStripeAmount(charge.amountRefunded, charge.currency)})
                    </span>
                  )}
                </div>
              </div>
      ...
      {sortedCharges.length > 0 && (
        <div className="flex items-center justify-between pt-4 border-t">
          ...
          <p className="text-sm text-muted-foreground">
            Showing {sortedCharges.length} transaction{sortedCharges.length !== 1 ? "s" : ""}
          </p>
          <Button
            variant="outline"
            size="sm"
