    ...
    dbPromise = openDB<InviteCodeDB>(DB_NAME, DB_VERSION, {
      upgrade(db) {
        ...
        if (!db.objectStoreNames.contains("user_profiles")) {
          const store = db.createObjectStore("user_profiles", {
            keyPath: "id",
          });
        ...
        if (!db.objectStoreNames.contains("credit_purchases")) {
          const store = db.createObjectStore("credit_purchases", {
            keyPath: "id",
          });
          store.createIndex("by-user-id", "userId");
        }
...
export const dbOperations = {
  async getAll<K extends keyof InviteCodeDB>(
    storeName: K
  ): Promise<InviteCodeDB[K]["value"][]> {
    const db = await getDB();
    return db.getAll(storeName);
  },

  async put<K extends keyof InviteCodeDB>(
    storeName: K,
    value: InviteCodeDB[K]["value"]
  ) {
    const db = await getDB();
    return db.put(storeName, value);
  },

  async putAll<K extends keyof InviteCodeDB>(
    storeName: K,
    values: InviteCodeDB[K]["value"][]
  ) {
    const db = await getDB();
    const tx = db.transaction(storeName, "readwrite");
    const store = tx.objectStore(storeName);
    await Promise.all([...values.map((v) => store.put(v)), tx.done]);
  },
  ...
  async deleteDatabase(): Promise<void> {
    return new Promise<void>((resolve, reject) => {
      ...
      request.onsuccess = () => {
        ...
        // Reset the dbPromise so it reinitializes on next access
      ...
      request.onerror = () => {
        console.error(
          "[Security] Error deleting IndexedDB database:",
          request.error
        );
      ...
      request.onblocked = () => {
        console.warn(
          "[Security] IndexedDB deletion blocked - database still in use"
        );
        // Still resolve since the deletion will complete once unblocked
        resolve();
      };
    });
  },
